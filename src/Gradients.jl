...
function ∇measure(F::Function, z::BorderedArray, θ::AbstractVector, targets::StateSpace; newton_options=NewtonPar(verbose=false, max_iterations=800, tol=1e-6))
    ∂implicit, _, _ = newton_options.linsolver(-∂Fz(F, z, θ)', gradient(z -> measure(F, z, θ, targets), z))
