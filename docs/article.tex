...
\begin{itemize}
    ...
    \item Implementation of the method as a Julia package\\  \href{https://github.com/anon/Code.jl}{\texttt{see attached zip file}}
...
\end{align}
The most common bifurcations between steady states, not including limit cycles, are saddle-nodes and pitchforks \cite{Haragus2011LocalSystems}. The relationships between our conditions for bifurcation \eqref{eq:predictions} and the conditions for saddle-node are detailed in Appendix \ref{appendix:conditions}. Saddle-node bifurcations, which often appear in pairs (Figure \ref{fig:saddle-node}) are defined by stable and unstable fixed points meeting and disappearing. Pitchfork bifurcations are special cases of the saddle-node where a single steady state splits into two stable and one unstable steady state (Figure \ref{fig:pitchfork}). To illustrate these bifurcations, we define minimal models (Figure \ref{fig:minimal-models}) that span the space of saddle-node and pitchforks, where indeed zero crossings in the determinant with a finite slope define the set of prediction $\predictions(\theta)$. The location of these crossings in general may not match the targets $\targets$.
...
% Finally, we note that $\measure(s)$ has the desirable property of being invariant to certain classes of transformations on $\rates(u,p)$ that do not change bifurcation locations (details provided in Appendix \ref{appendix:measure}). 
