...
A saddle-node bifurcation is the generic bifurcation of equilibria and additional conditions are required at a bifurcation point to obtain a transcritical or pitchfork bifurcations \cite{Kuznetsov2004TopologicalSystems}. If we are not specifically looking for transcritical or pitchfork bifurcations, it is sufficient to consider a non-zero Hessian determinant and non-zero vector slope
\begin{equation}
    \left|\frac{\partial^2\rates}{\partial u^2}\right| \neq 0\qquad \frac{\partial\rates}{\partial p}\neq 0
    \label{eq:conditions}
\end{equation}
In the multidimensional case we would like to express the above conditions succinctly in terms of a scalar derivative. Let us consider the non-degeneracy condition along the equilibrium manifold in terms of the state-space determinant
\begin{equation}
    \frac{d}{ds}\left|\frac{\partial\rates}{\partial u}\right|\neq 0
\end{equation}
Applying Jacobi's formula and expanding the directional derivative in terms of basis vectors $\hat{u}(s)$ and $\hat{p}(s)$ that are tangent to the equilibrium manifold at a particular location $s$ we get
\begin{equation}
     \hat{p}(s)\,\mathrm{Tr}\left[
        \frac{\partial^2\rates}{\partial u\partial p}
        \,\mathrm{adj}\frac{\partial\rates}{\partial u}
        \right]
        +
    \sum_{n=1}^N\hat{u}_{n}(s)\,\mathrm{Tr}\left[
        \frac{\partial^2\rates}{\partial u \partial u_n}
        \,\mathrm{adj}\frac{\partial F}{\partial u}
        \right]
  \neq 0
\end{equation}
We can see this expression contains the vector Hessian $\frac{\partial^2\rates}{\partial u^2}$ whos determinant is non-zero for the non-degenerate case. The slope $\frac{\partial\rates}{\partial p}$ is non-zero when the transversality condition holds. 
